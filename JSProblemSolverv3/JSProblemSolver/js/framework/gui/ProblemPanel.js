function ProblemPanel(problem) {
	...
	messageDisplay.css("font-size", 'xx-small');
	messageDisplay.css("color", 'White');
	messageDisplay.css("background-color", 'Crimson');
	messageDisplay.animate({fontSize: '1.5em'}, "slow");
	stateDisplay.text("Solved");
	panel.css("color", 'Crimson');
		var congrats = $("<div></div>");
		congrats.addClass("centerText");
		congrats.text("Congrats The Problem is Solved");
		panelChange();
		return congrats;
	}
